...
export function KothGame({ pool, onExit }: { pool: Movie[]; onExit: () => void }) {
  ...
  const { results, challenges } = game;
  const done = phase === "done";

  const deriveStep =
    done ? results.length : phase === "advance" ? animStep + 1 : animStep;
  const { king, challenger, streak } = kothStateAt(seeds, results, deriveStep);
  const revealed = results[animStep] != null;
  const winner: Winner | null = results[animStep] ?? null;

  const processed = phase === "advance" ? animStep + 1 : animStep;
  const challengers: Challenger[] = seeds.slice(1).map((movie, i) => {
    let state: ChallengerState;
    if (movie.id === king.id) state = "king";
    else if (i < processed) state = "defeated";
    else if (i === animStep && (phase === "focus" || phase === "voting"))
      state = "current";
    else state = "upcoming";
    return { movie, state };
  });
  ...
  const label =
    phase === "seeding"
      ...
      : phase === "intro"
        ...
        : done
          ? "Winner"
          : `CHALLENGE ${animStep + 1} OF ${challenges}`;

  const kingUri = posterUri(king.posterPath);
    ...
    <View style={styles.root}>
      ...
      {phase === "voting" && challenger ? (
        <DuelVote
          ...
          left={king}
          right={challenger}
          label={`CHALLENGE ${animStep + 1} OF ${challenges}`}
          counts={game.counts}
          ...
          winner={winner}
          tiebreak={
            revealed && game.counts.left === game.counts.right ? "hold" : null
          }
          onVote={castVote}
      ...
      {done ? (
        ...
        >
          ...
          <View style={styles.winnerPoster}>
            {kingUri ? (
              <Image source={{ uri: kingUri }} style={styles.winnerImg} contentFit="cover" />
            ) : (
          ...
          <Text style={styles.winnerSub}>rules the hill</Text>
        </Animated.View>
      ) : null}
